...
exports.handler = async (event, context) => {
  ...
  const orderDetails = `
Order Details:
${cartItems.map(item => `Product: ${item.name}, Quantity: ${item.quantity}, Price: ${item.price}`).join('\n')}
...
Postal Code: ${postalCode}
  ...
  try {
    ...
    let transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.GMAIL_USER, // Set this in Netlify environment variables
        pass: process.env.GMAIL_PASS, // Set this in Netlify environment variables
      },
    });
    ...
    await transporter.sendMail({
      ...
      to: 'arafeys12@gmail.com', // New target email address
