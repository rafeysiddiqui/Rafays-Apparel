...
const FormalPage = () => {
  ...
  const products = Array(numberOfProducts).fill().map((_, i) => ({
    id: `formal-${i + 1}`,
    name: `Formal Design ${i + 1}`,
    price: 250 + (i * 25),
    media: [{ type: 'image', url: 'https://via.placeholder.com/400x600/333333/ffffff?text=Formal+' + (i + 1) }],
    description: 'A sophisticated design for formal occasions.'
  }));
