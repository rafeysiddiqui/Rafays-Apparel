...
export default function Navigation({ isOpen, setIsOpen, activeDropdown, setActiveDropdown, menuRef, categories }) {
  const { cartItems } = useCart();

  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-4xl px-4">
      <div className="bg-gradient-to-r from-purple-900 via-black to-black rounded-full shadow-xl py-4 px-8 flex justify-between items-center">
        ...
        <div
          ref={menuRef}
          className={`fixed top-20 right-4 w-64 bg-gray-800 rounded-lg shadow-xl transition-all duration-300 ${
